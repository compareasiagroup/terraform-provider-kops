...
inputs = {
  ...
  cluster_name          = local.account_vars.foo_blue_kube_cluster
  vpc                   = dependency.network.outputs.vpc
  utility_subnets       = dependency.network.outputs.public_subnets.subnets_map
  node_subnets          = dependency.network.outputs.kube_cluster_blue_subnets.subnets_map

  # sample of centralized DNS zones
  extdns_zones          = [
    dependency.network.outputs.internal_zones[local.account_vars.foo_blue_kube_cluster],
  ]

  master_roles          = [
    "arn:aws:iam::${local.root_vars.accounts["account-1"]}:role/externaldns.${local.root_vars.foo_dns}",
    "arn:aws:iam::${local.root_vars.accounts["account-1"]}:role/externaldns.${local.root_vars.bar_dns}",
  ]
